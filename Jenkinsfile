pipeline {
    ...
    environment {
        IMAGE_NAME = "introduction"
        IMAGE_TAG = "${BUILD_NUMBER}"
        CONTAINER_NAME = "adeola-introduction"
        ENV_NAME = "dev"
        ...
        stage('SonarQube scan') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectKey=introduction -Dsonar.projectName=introduction"
                }
            }
        }
        stage("Docker Build") {
